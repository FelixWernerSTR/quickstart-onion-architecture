...
public class ReportWorkerWorkLogState implements ConsoleState {
  ...
  private Worker searchWorker() throws IOException {
    ...
    try {
      ...
      System.out.println(String.format("Worker found. Details:%nLogin: %s%nName: %s%nSurname: %s%nEmail: %s%n%n", foundedWorker.getLogin(),
          foundedWorker.getName(), foundedWorker.getSurname(), foundedWorker.getEmail()));
