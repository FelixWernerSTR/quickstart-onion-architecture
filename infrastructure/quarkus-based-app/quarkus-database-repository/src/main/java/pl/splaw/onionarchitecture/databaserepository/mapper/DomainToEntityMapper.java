...
public class DomainToEntityMapper {
  ...
  public WorkLogEntity workLogToWorkLogEntity(WorkLog workLog) {
    return new WorkLogEntity(workLog.getStartDate(), workLog.getTimeSpentInSeconds(), workLog.getDescription(),
        workerToWorkerEntity(workLog.getAssociatedWorker()));
  }
  
  public WorkLogEntity workLogToWorkLogEntityWithId(WorkLog workLog) {
    return new WorkLogEntity(workLog.getWorkLogId(), workLog.getStartDate(), workLog.getTimeSpentInSeconds(), workLog.getDescription(),
        workerToWorkerEntity(workLog.getAssociatedWorker()));
  }
  
  public WorkLog workLogEntityToWorkLog(WorkLogEntity workLogEntity) {
    return new WorkLog(workLogEntity.getId(), workLogEntity.getStartDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate(),
        workLogEntity.getTimeSpentInSeconds(), workerEntityToWorker(workLogEntity.getAssociatedWorker()), workLogEntity.getDescription());
  }
